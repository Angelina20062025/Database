...
namespace MusicStore
{
    public partial class SellRecordForm : Form
    {
        ...
        private string GenerateReceipt(int purchaseId, int customerId)
        {
            try
            {
                string receipt = "         МАГАЗИН ПЛАСТИНОК\n";
        ...
        private void btnAddToCart_Click(object sender, EventArgs e)
        {
            ...
            try
            {
                ...
                // Получаем актуальную информацию о пластинке из базы данных
                decimal price = 0;
                int available = 0;
                ...
                using (var conn = new NpgsqlConnection(connString))
                {
                    ...
                    using (NpgsqlDataReader reader = cmd.ExecuteReader())
                    {
                        ...
                        {
                            MessageBox.Show("Пластинка не найдена в базе данных", "Ошибка", MessageBoxButtons.OK, MessageBoxIcon.Error);
                            return;
                        }
                    }
                }
                ...
                // Проверяем, есть ли уже такая пластинка в корзине
